...
class ShapesThatMustNotChangeTest(TempFilesTestCase):
    ...
    def test_a_proteomics_style_symbol_first_row_is_still_a_feature(self):
        """datasets/08's proteomics_relevant.tab starts with a gene symbol,
        which fails the ID heuristic. It has no `#`, so it is data, not a
        header, and must survive the parse.

        The symbol is read back from the file rather than named here. This
        assertion used to name `Nup50`; the datasets were regenerated by
        `python -m src.AdminTools.scripts.exampledata`, the first row became
        `Acaca` and Nup50 left the file altogether -- which is precisely the
        trap firstDataRow() above was written for, and precisely why this
        suite has been carried in run_all.BASELINE as "2 failures" since.
        """
        path = dataset("08-stategra-multiomics", "proteomics_relevant.tab")
        firstFeature = firstDataRow(path)[0].lower()
...
class BedCommentRowsTest(TempFilesTestCase):
    ...
    def test_the_shipped_bed_relevance_file_parses_losslessly(self):
        """No key is built from a comment row, and row one survives.

        This was test_the_shipped_stray_header_is_not_a_relevant_region, and it
        named the region `17_13654752_13655272` because datasets/09 carried a
        stray `#CHR<TAB>start<TAB>end` at line 886. The datasets were
        regenerated: the stray header is gone from the file, and so is that
        region, so the test failed on data that had legitimately moved.

        The comment-row defect it was written for is not lost -- it is covered
        by test_a_leading_bed_header_is_skipped_too below, which constructs the
        shape instead of depending on a shipped file still happening to have
        it. That is the more honest home for it: a regression test should not
        go quiet because a data generator stopped emitting the bad row.
        """
        path = dataset("09-stategra-regions", "dnase_unmapped_relevant.tab")
