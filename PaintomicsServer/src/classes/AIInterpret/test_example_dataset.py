...
def _build_example_job(databases, experiment_design, tmp_dir):
    ...
    for omic_name, enrichment in EXAMPLE_OMICS.items():
        data_file = example_dir + omic_name.replace(" ", "_").replace("-seq", "").lower() + "_values.tab"
        relevant_file = example_dir + omic_name.replace(" ", "_").replace("-seq", "").lower() + "_relevant.tab"
        payload = {
            "omicName": omic_name,
            "inputDataFile": data_file,
            "relevantFeaturesFile": relevant_file,
            "isExample": True,
