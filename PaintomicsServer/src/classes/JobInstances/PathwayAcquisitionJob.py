...
class PathwayAcquisitionJob(Job):
    ...
    def generatePathwaysList(self):
        ...
        def matchPathways(jobInstance, pathwaysList, genesInAllPathways, compoundsInAllPathways, inputGenes, inputCompounds, totalFeaturesByOmic, totalRelevantFeaturesByOmic, matchedPathways, mappedRatiosByOmic, enrichmentByOmic):
            #****************************************************************
        ...
        for feature in chain(self.getInputCompoundsData().values(), self.getInputGenesData().values()):
            ...
            if feature.getID() in totalFeatures.get(feature.getMatchingDB()):
                for omicValue in feature.getOmicsValues():
                    ...
                    counterNames[feature.getMatchingDB()][omicValue.getOmicName()][enrichmentProperty] = (counterNames[feature.getMatchingDB()][omicValue.getOmicName()][enrichmentProperty] or relevantValue)
            else:
    ...
    def testPathwaySignificance(self, genesInPathway, compoundsInPathway, inputGenes, inputCompounds, totalFeaturesByOmic, totalRelevantFeaturesByOmic, mappedRatiosByOmic, enrichmentByOmic, sourceDB):
        """
    ...
    def compundsClassification (self):
        ...
        for key in classificationDict:
            pValueInDict[key] = round(calculateSignificance(self.test, totalFeatures, totalRelevantFeatures, totalFeaturesInCategory.get(key), totalRelevantFeaturesInCategory.get(key)), 4)
        ...
        return dict(mappingComp), dict(pValueInDict), dict(classificationDict), dict(exprssionMetabolites), dict(adjustPvalue), dict(totalRelevantFeaturesInCategory), featureSummary, compoundRegulateFeatures


    def globalExpressionData(self):
        globalExpressionDataset = defaultdict(dict)
        for i in self.inputGenesData:
            ...
            globalExpressionDataset[expressionID] = expressionDetail

        return globalExpressionDataset
